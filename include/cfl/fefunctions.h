...
struct is_summable<A, B, typename std::enable_if<
                           fe_function_set_type<A>::value != ObjectType::none &&
...
template <template <int, int, unsigned int> class T, int rank, int dim, unsigned int idx>
struct test_function_set_type<
  T<rank, dim, idx>, std::enable_if_t<std::is_base_of<Base::TestFunctionFaceBase<T<rank, dim, idx>>,
                                                      T<rank, dim, idx>>::value>>
{
...
struct IntegrationFlags
{
  ...
  constexpr bool operator&(const IntegrationFlags& other_flags) const
  {
    return (value & other_flags.value) || (value_exterior & other_flags.value_exterior) ||
           (gradient & other_flags.gradient) || (gradient_exterior & other_flags.gradient_exterior);
  }
};
...
public:
  ...
  */
  ...
  {
    return ProductFEFunctions<NewFEFunction, FEFunction, Types...>(new_product.get_factor(), *this);
  }
